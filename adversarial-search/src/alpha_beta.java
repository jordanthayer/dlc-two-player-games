...
/** AIPlayer using Minimax algorithm
    Adapted (mostly copied) from
    Dong Xiang, "Solve Tic Tac Toe with the MiniMax algorithm"
    http://www.codeproject.com/Articles/43622/Solve-Tic-Tac-Toe-with-the-MiniMax-algorithm.
 */
public class AIPlayerMinimax extends AIPlayer {
    private static final boolean runAB = true;
   ...
   @Override
   int[] moveAB() {
       int[] result = alphabeta(2, mySeed, Integer.MIN_VALUE, Integer.MAX_VALUE);
       // depth, max-turn, alpha, beta
       return new int[] {result[1], result[2]};   // row, col
   }

    /** Alphabeta (recursive) at level of depth for maximizing or minimizing player
        with alpha-beta cut-off. Return int[3] of {score, row, col}  */
    private int[] alphabeta(int depth, Seed player, int alpha, int beta) {
        ...
        if (nextMoves.isEmpty() || depth == 0) {
            ...
            for (int[] move : nextMoves) {
                ...
                if (player == mySeed) {  // mySeed (computer) is maximizing player
                    score = alphabeta(depth - 1, oppSeed, alpha, beta)[0];
                    if (score > alpha) {
                        alpha = score;
                        bestRow = move[0];
                        bestCol = move[1];
                    ...
                    score = alphabeta(depth - 1, mySeed, alpha, beta)[0];
                    if (score < beta) {
                        beta = score;
                        bestRow = move[0];
                        bestCol = move[1];
